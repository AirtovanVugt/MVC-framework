...
function createEmployee($data){
    ...
    try {
        $conn = openDatabaseConnection();
        $query = $conn->prepare("INSERT INTO employees (name, age) VALUES (:naam, :jaar)"); 
        $query->execute([":naam" => $data["Naam"], ":jaar" => $data["Jaar"]]);
        $conn = null;
 ...
 function updateEmployee($data){
    ...
    $conn = openDatabaseConnection();
    $query = $conn->prepare("UPDATE employees SET name = :naam, age = :jaar WHERE id = :id");
    $query->execute([":id" => $data["id"], ":naam" => $data["Naam"], ":jaar" => $data["Jaar"]]);
    $conn = null;
 }
